...
class RosselandTest(unittest.TestCase):
    ...
    def test_extract_ktable(self):
        k_ext, _ = self.model.extract_ktable(P_input=1 * u.bar, T_input=1000 * u.K, band=(0.3 * u.um, 50 * u.um))
        ...
        k_true = self.model.ktable[idp][idt][idw]
        self.assertListEqual(list(k_true.value), list(k_ext.value))

        P_input = [1, 10] * u.bar
        T_input = [1000, 2000] * u.K
        k_ext, _ = self.model.extract_ktable(P_input, T_input, band=(0.3 * u.um, 50 * u.um))

        for i, p in enumerate(P_input):
            for j, t in enumerate(T_input):
                k_true, _ = self.model.extract_ktable(p, t, band=(0.3 * u.um, 50 * u.um))
